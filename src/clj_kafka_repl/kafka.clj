...
(defn consume!
  "Consumes the next message (if any available) from the given tracked channel created with [[kafka/consumer-chan]]."
  [ch]
  (ch/poll! ch))

(s/fdef consume!
        :args (s/cat :ch ::ch/consumer-channel)
        :ret any?)

(s/fdef consume!
        :args (s/cat :ch (s/or :consumer-channel ::ch/consumer-channel
                               :channel ::ch/channel))
        :ret any?)
